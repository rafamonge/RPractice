...
## * download and install it Also helpful to install RStudio (download
## * from [http://rstudio.com])
...
## * [http://tutorials.iq.harvard.edu/R/Rgraphics.zip] Extract the zip
## * file containing the materials to your desktop

##   Workshop notes are available in .hmtl format. Open a file
##   browser, navigate to your desktop and open Rgraphics.html
...
##   My goal: by the end of the workshop you will be able to reproduce
...
library(ggrepel)
ggplot(dat, aes(x = CPI, y = HDI, color=Region)) + geom_point(aes(size = HDI.Rank), alpha=0.5) + geom_text_repel(aes(label=Country))
...
myColors = c("Americas" = "red", "Asia Pacific" = "blue", "East EU Cemt Asia" = "black", "EU W. Europe" ="green", "MENA" = "orange", "SSA" = "purple")
ggplot(dat, aes(x = CPI, y = HDI, color = Region)) +
  geom_point() +
  scale_x_continuous(name = "CPI!!") + 
  scale_y_continuous(name = "Human Development Index") +
  scale_color_manual(name="MyRegions", values = myColors)
...
library(ggthemes)
target = c("India", "Costa Rica", "Spain", "France" )
datFiltered = dat  %>% 
...
ggplot(dat, aes(x = CPI, y = HDI)) +
  ...
  geom_text_repel(data = datFiltered, aes(label=Country)) +
